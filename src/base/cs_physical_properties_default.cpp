...
#pragma omp parallel for if (n_cells > CS_THR_MIN)
      ...
      if (iebdfm) {
        cs_real_6_t *vistes = (cs_real_6_t *)cs_field_by_name(
                                "anisotropic_turbulent_viscosity_scalar")
                                ->val;
...
#pragma omp parallel for if (n_cells > CS_THR_MIN)
          for (cs_lnum_t c_id = 0; c_id < n_cells; c_id++) {
            ...
            const int       c_act = cs_mesh_quantities_cell_is_active(mq, c_id);
            const cs_real_t rottke =
              cs_turb_csrij * crom[c_id] * xttdrb * c_act;
                ...
                const cs_real_t             brom[])
{
  ...
  const char *f_names[] = { CS_F_(rho)->name,
                            CS_F_(mu)->name,
                            CS_F_(mu_t)->name,
                            "" };
...
static void
_check_log_mesh_diff(bool first_pass, cs_lnum_t n_cells)
{
  ...
  if (eqp->idften & CS_ANISOTROPIC_LEFT_DIFFUSION) {
    ...
    for (int ii = 0; ii < 6; ii++) {
      ...
      if ((eqp->verbosity > 0) || (first_pass) || (varmn < 0.0)) {
        ...
        bft_printf("  %s           %10.12e       %10.12e", tmp_s, varmn, varmx);
      }
      ...
      if (varmn < 0.0) {
        bft_error(__FILE__,
                  ...
                  _("Warning: abort in the physical quantities computation\n"
                    ...
                    "Verify he definition of this property"),
                  varmn);
      }
    }
  }
...
static void
_physical_properties_update_models_stage_1(int iterns, int *mbrom)
{
...
cs_physical_properties_update(int iterns)
{
  ...
  // Densities at boundaries are computed in cs_vof_compute_linear_rho_mu for
  // VoF
  ...
  // BEFORE the user
  _physical_properties_update_models_stage_1(iterns, &mbrom);
...
#pragma omp parallel for if (n_b_faces > CS_THR_MIN)
  ...
  if (cs_glob_time_step->nt_cur == cs_glob_time_step->nt_prev + 1) {
    if (cs_glob_fluid_properties->irovar == 0) {
      ...
      _field_is_constant("the density", n_cells, CS_F_(rho)->val, ro0);
